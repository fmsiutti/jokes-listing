...
<template>
  <div class="flex justify-end w-full pt-6 pr-6 dark:bg-primary bg-white ease-linear duration-100">
    <CustomButton @click="toggleJokeForm" type="primary" size="md" >
      Submit your own Joke
    </CustomButton>
    <div class="w-1/2 z-20 h-72 rounded-lg bg-white border p-4 border-light dark:border-lighter absolute z-10 scale-0 opacity-0 ease-out duration-200 origin-top-right" :class="{'opacity-100 scale-100': jokeFormOpen}">
  ...
  <div class="items-end sm:items-stretch min-h-fullscreen flex flex-col sm:flex-row sm:space-x-4 p-6 dark:bg-primary transition-colors ease-linear duration-100 relative">
    <CustomButton @click="toggleFilters" type="secondary-full" circle class="sm:hidden sticky z-10 top-4 right-4">
