...
const { randomJoke, randomTen, randomSelect, jokeByType, jokeById, getPaginated, jokesCount } = require('./handler');
...
app.get('/jokes_count', (req, res) => {
  res.json(jokesCount());
});

app.get('/jokes_count/:type', (req, res) => {
  res.json(jokesCount(req.params.type));
});
...
app.get('/jokes/paginated/:count/:page', (req, res, next) => {
  try {
    const { count, page } = req.params;
    const jokes = getPaginated(count, page)
    if (!jokes) return next({ statusCode: 404, message: 'joke not found' });
...
app.get('/jokes/paginated/:count/:page/:type', (req, res, next) => {
  try {
    const { count, page, type } = req.params;
    const jokes = getPaginated(count, page, type)
    if (!jokes) return next({ statusCode: 404, message: 'joke not found' });
    return res.json(jokes);
  } catch (e) {
    return next(e);
